...
if((param.contact == 1) & (param.dimensionPlot3D == 0))
    % Inside of the pipe
    line([-pipeLength pipeLength],[0.025+param.d/2 0.025+param.d/2],'color','black');
    hold on
    line([-pipeLength pipeLength],[-0.025-param.d/2 -0.025-param.d/2],'color','black');
    hold on
    
    % Outside of the pipe
    line([-pipeLength pipeLength],[0.025+param.diameter/2 0.025+param.diameter/2],'color','black', 'linewidth', 5); 
    hold on
    line([-pipeLength pipeLength],[-0.025-param.diameter/2 -0.025-param.diameter/2],'color','black', 'linewidth', 5);
    hold on
elseif((param.contact == 1) & (param.dimensionPlot3D == 1))
    % 3D pipe
    plotcube([3,0.1,param.d],[-2,-param.diameter-0.025,0],0.3,[0 1 0]);
    hold on
    plotcube([3,0.1,param.d],[-2,param.diameter+0.025,0],0.3,[0 1 0]);
    hold on
    plotcube([3,2*param.diameter-0.05,0.01],[-2,-param.diameter+0.075,0],0.3,[0 1 0]);
    hold on
    camlight;
    lighting phong;
    hold on
...
for i=1:shotsNumber:length(X(:,1)) 
    if(param.dimensionPlot3D == 1)
        ...
        for k=1:param.N
            ...
            if(((ycLink(k,i)>(param.d/2)) | (ycLink(k,i)<(-param.d/2))) & (param.contact == 1)) 
        ...
        if param.contact == 1
             % Inside of the pipe
            line([-pipeLength pipeLength],[0.03+param.d/2 0.03+param.d/2],'color','black');
            hold on
            line([-pipeLength pipeLength],[-0.03-param.d/2 -0.03-param.d/2],'color','black');
            hold on
            
            % Outside of the pipe
            line([-pipeLength pipeLength],[0.03+param.diameter/2 0.03+param.diameter/2],'color','black', 'linewidth', 3); 
            hold on
            line([-pipeLength pipeLength],[-0.03-param.diameter/2 -0.03-param.diameter/2],'color','black', 'linewidth', 3);
            hold on
        end
        ...
        axis([-pipeLength pipeLength -0.8 0.8]);
        axis equal
        pause(.001);
